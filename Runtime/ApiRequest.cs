...
namespace ApiHandling.Runtime
{
    ...
    {
        UniTask<Result<string>> GetRequest(string path, CancellationToken cancellationToken = default);
        UniTask<Result<string>> PostRequest(string path, string requestBody = "", CancellationToken cancellationToken = default);
        UniTask<Result<string>> PatchRequest(string path, string jsonBody, CancellationToken cancellationToken = default);
    ...
    public class TestApiRequest : IApiRequest
    {
        ...
        public TestApiRequest(string url)
        {
            _url = url;
        }
        private string Url(string path) => $"{_url}/{path}";
        ...
        public async UniTask<Result<string>> PostRequestForm(string path, CancellationToken cancellationToken = default, params FormItem[] formItems)
        {
            ...
            foreach (var item in formItems)
            {
                ...
                {
                    return Result<string>.Failure(EResultError.NullValue, $"Null form item of name: {item.Key}");
                }
                switch (item.Type)
                {
